...
class _PersonalInfoPageState extends State<PersonalInfoPage> {
  ...
  final _emailController = TextEditingController(text: "ange.angemurielle@gmail.com");
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Personal Information', style: TextStyle(color: textColor, fontWeight: FontWeight.bold, fontSize: 22)),
        centerTitle: true,
      ),
          ...
          children: [
            ...
            Stack(
              children: [
                ...
                CircleAvatar(
                  radius: 75,
                  backgroundColor: lightGreenAccent.withOpacity(0.1),
                  child: ClipOval(
                    child: _uploadedImage != null
                        ? Image.file(_uploadedImage!, fit: BoxFit.cover, width: 150, height: 150)
                        // Requirement: Default image is the local asset for ange
                        : Image.asset('assets/images/ange1.jpeg', fit: BoxFit.cover),
                  ),
                ),
                ...
                Positioned(
                  ...
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: const Icon(Icons.photo_camera_rounded, color: brandGreen, size: 24),
                    ),
                  ),
                ),
              ],
            ...
            const Text("ANGE AWAGOUM", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 22, color: textColor)),
            const Text("AGROLYNC MEMBER", style: TextStyle(color: brandGreen, fontWeight: FontWeight.bold, fontSize: 14, letterSpacing: 0.5)),
            const SizedBox(height: 35),

            // --- Multi-Step Information Form ---
            _buildStyledTextField(label: "Full Name", controller: _nameController, icon: Icons.person),
            const SizedBox(height: 20),
            _buildStyledTextField(label: "Phone Number", controller: _phoneController, icon: Icons.phone),
            const SizedBox(height: 20),
            _buildStyledTextField(label: "Email", controller: _emailController, icon: Icons.email),
          ],
        ),
      ),
          ...
          child: Column(
            ...
            children: [
              SizedBox(
                ...
                width: MediaQuery.of(context).size.width * standardButtonWidthRatio, // Standardized width
                child: ElevatedButton(
                  ...
                  child: Row(
                    ...
                    children: const [
                      Icon(Icons.check_circle_outline_rounded, size: 22, color: Colors.white70),
                      SizedBox(width: 10),
                      Text("Update Profile", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                    ],
                  ),
                ),
              ),
  ...
  Widget _buildStyledTextField({required String label, required TextEditingController controller, required IconData icon}) {
    ...
    return Column(
      ...
      children: [
        Text(label, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: textColor)),
