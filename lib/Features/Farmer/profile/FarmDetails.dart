...
class _FarmDetailsScreenState extends State<FarmDetailsPage> {
  ...
  final _locationController = TextEditingController(text: "Behind Muea Market, Buea, Cameroon");
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: Stack(
          children: [
            GoogleMap(
              initialCameraPosition: CameraPosition(target: _pickedLocation, zoom: 14.0),
              ...
              child: Container(
                ...
                child: const Row(
                  ...
                  children: [
                    ...
                    SizedBox(width: 6),
                    Text("Adjust Pin", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                  ],
                ),
              ),
            ),
  ...
  Widget _buildLabel(String label) => Padding(
    ...
    child: Text(label, style: const TextStyle(fontWeight: FontWeight.bold, color: subTextColor, fontSize: 14)),
  );

  Widget _buildTextField({required TextEditingController controller, String? suffixText}) {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(child: TextField(controller: controller, decoration: const InputDecoration(border: InputBorder.none))),
          if (suffixText != null) Text(suffixText, style: const TextStyle(color: subTextColor, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
      ...
      child: Row(
        children: [
          Expanded(
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: TextField(controller: _locationController, decoration: const InputDecoration(border: InputBorder.none)),
            ),
          ),
          const Padding(
  ...
  Widget _buildDropdown() {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), border: Border.all(color: fieldBorderColor)),
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          value: _selectedCrop,
          isExpanded: true,
          items: ["Corn & Soybeans", "Wheat", "Coffee","Tomatoes", "Pepper","Onions","Cabbage"].map((v) => DropdownMenuItem(value: v, child: Text(v))).toList(),
